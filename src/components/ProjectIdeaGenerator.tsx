        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 md:mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/10 text-accent text-xs md:text-sm font-bold mb-4 md:mb-6">
              ...
              {t('ideaGenerator.badge')}
            </div>
            <h2 className="font-headline font-bold text-3xl md:text-5xl mb-4">{t('ideaGenerator.title')}</h2>
            <p className="text-muted-foreground text-base md:text-lg">
              {t('ideaGenerator.desc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-12 items-start">
            <form onSubmit={handleGenerate} className="lg:col-span-2 space-y-5 md:space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('ideaGenerator.labelBusiness')}</label>
                <Input 
                  placeholder={t('ideaGenerator.placeholderBusiness')} 
                  value={businessType}
              ...
              <div className="space-y-2">
                <label className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('ideaGenerator.labelGoals')}</label>
                <Textarea 
                  placeholder={t('ideaGenerator.placeholderGoals')} 
                  value={businessGoals}
            ...
            <div className="lg:col-span-3 min-h-[250px] md:min-h-[300px] flex flex-col">
              {!suggestions && !loading ? (
                <div className="flex-1 border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center text-center p-8 md:p-12 opacity-30">
                  ...
                  <p className="text-sm md:text-base">{t('ideaGenerator.placeholderResult')}</p>
                </div>
              ) : loading ? (
                <div className="flex-1 flex flex-col items-center justify-center space-y-4 py-12">
                  <Loader2 className="animate-spin text-primary" size={32} />
                  <p className="text-muted-foreground animate-pulse text-sm md:text-base">{t('ideaGenerator.loading')}</p>
                </div>
              ) : (
                  ...
                  {suggestions?.suggestions.map((item, idx) => (
                    <Card key={idx} className="bg-white/5 border-white/10 hover:border-accent/30 transition-colors">
                      ...
                      <CardContent className="p-4 md:p-6 pt-0 md:pt-0">
                        ...
                        <p className="text-[10px] md:text-xs text-muted-foreground italic leading-relaxed">{t('ideaGenerator.why')}: {item.relevanceReason}</p>
                      </CardContent>
                    </Card>
                  ))}
                </div>
